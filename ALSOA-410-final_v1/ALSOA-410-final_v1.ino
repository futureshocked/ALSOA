...
const int closeness         = 50; // This value defines when the left and right light
...
void loop(){
  ...
  else    // The path is blocked, so the robot will use the distance sensor to find a clear path
  {
      ...
      panMotor.write(0);           // Turn the distance sensor to the right
      delay(500);                  // Wait for the sensor to move into position
      leftDistance = ping();       // scan to the right
      delay(500);                  // Wait for the sensor to move into position
      panMotor.write(180);         // Turn the distance sensor to the left
